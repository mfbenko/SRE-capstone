...
class TestKafkaConsuner(unittest.TestCase):
	...
	'''def test_insert_into_mongodb(self):
		message ={'': 'Anomalous', 'Method': '', 'User-Agent': '', 'Pragma': '', 'Cache-Control': '', 'Accept': '', 'Accept-encoding': '', 'Accept-charset': '', 'language': '', 'host': '', 'cookie': '', 'content-type': '', 'connection': '', 'lenght': '', 'content': '', 'classification': '', 'URL': '', {_id:{$regex : "^[a-fA-F0-9]{24}$"}}}	
		self.consumer_instance.insert_into_mongodb(message)
		self.mock_collection.insert_one.assert_any_call(message)
		self.consumer_instance.mongo_client.close()
	'''	
